...
//        dd($newestStore);
        $rsdata = DB::select( DB::raw("
                ...
                s.*
					...
					AND c.store_id in $listStoreId
                    ORDER BY random() DESC 
                    LIMIT 30
            "));// OR s.show_in_homepage = 1 OR s.sticky = 1
        $topCoupons = array_slice($rsdata,0,8);
	$rsdata = array_slice($rsdata,8);
        $data = [];
        //$data['newestStores'] = $newestStore;
        $data['sliderStores'] = $newestStore;
        $data['couponList'] = $rsdata;
        $data['topCoupons'] = $topCoupons;
        $data['sliders'] = [];
        $data['topCashBack'] = [];
		...
		*/
        ...
        return view('v2-welcome')->with(array_merge($data, $dataSeo, $dataPage));
    }
