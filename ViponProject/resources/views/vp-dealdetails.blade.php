...
50% OFF</span>
        ...
        <script type="text/javascript">
            ...
            //        },
            //        success: function (data) {
            //            if ((data.code == 200 || data.code == 403) && data.data.group_buy) {
            //                $('.group-span').css('display', 'block');
            //                if (data.data.data.group_buy_final_price <= 0) {
            //                    $('#groupPrice').html('FREE');
            //                } else {
            //                    $('#groupPrice').html(data.data.data.currency_show + (data.data.data.group_buy_final_price).toFixed(2));
            //               }
            //            } else {
            //                $('.group-span').css('display', 'none');
            //            }
            ...
            document.getElementById('shareBtn').onclick = function () {
                FB.AppEvents.logEvent("shareThisProduct");
                FB.ui({
                    app_id: '',
                    method: 'share',
                    display: 'popup',
                    quote: '',
                    href: 'https://www.vipon.com/product/5360190'
                }, function (response) {
                });
            };
            ...
            //  $.ajax({
            //    url: "/search/feature-deal?filter=5360190&domain=www.amazon.com&type=product",
            //  method: 'GET',
            //cache: true,
            ...
            // });
            // });
            //validate product id by john 22/9/17
            var validate_product_id = '';

            function validate(product_id) {
                validate_product_id = product_id;
                grecaptcha.execute();
                <!-- BingConversion -->
                bing_productpage_instant_deal();
                <!-- BingConversion end -->
                fbq('track', 'instant deal');
            }

            //validate callback by john 22/9/17
            function onSubmit(token) {
                // request_now(validate_product_id,token);
            }

            //request_now edit by john 22/9/17
            function request_now(product_id, token) {
...
//                            $('#thumbs-up').html(data.data.up);
            ...
            $(function () {
                ...
                countdown = setInterval(function () {
                    ...
                    if (secs == 0) {
                        $('.request-btn').html('<span class="glyphicon glyphicon-time"></span> <span>Expired</span>');
            ...
            //显示
            function show_suspend_sellers(product_id) {
                //ga('send', 'event', 'Deal_home', 'add_deal', product_id);
                //$('#Suspend_Sellers .modal-body').html('<div style="padding: 20px 20px 0 20px;"><i class="fa fa-spin fa-spinner"></i> Please Wait...</div>');
                if (product_id == '') {
                    return false;
                }

                var productId = parseInt(product_id);
                $("#model_id").val(productId);
                $('#Suspend_Sellers').modal('show');
            ...
            function suspend() {
                ...
                var product_id = parseInt($("#model_id").val());
                if (product_id == '') {
                    alert("param is wrong");
                    return false;
                }

                var suspend_reason = $.trim($("#suspend_reason").val());
                if (suspend_reason == '') {
                    alert("please enter the reason");
                    return false;
                }
                $("#Suspend_Sellers .btn").html('<i class="fa fa-spin fa-spinner"></i> Please Wait...');
                ...
                $.ajax({
                    ...
                    cache: false,
                    dataType: 'json',
                    data: {product_id: product_id, reason: suspend_reason},
                    success: function (data) {

                        if (data['status'] == 200) {
                            $('#Suspend_Sellers').modal('hide');
            ...
            function give_a_like(product_id, p) {
                if (product_id == '') {
                    alert("param is wrong");
                    return false;
                }
                ...
                $.ajax({
                    ...
                    cache: false,
                    dataType: 'json',
                    data: {product_id: product_id, type: p},
                    success: function (data) {
                        if (data['status'] == 200) {
                            if (p == 1) {
                                $('#thumbs-up-tips').html(data.message);
                            } else {
                                $('#thumbs-down-tips').html(data.message);
                            }
                        } else {
                            alert(data.message);
                        }
